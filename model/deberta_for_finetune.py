...
class DebertaV2(DebertaV2PreTrainedModel):
    ...
    def forward(
        ...
        labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
            Labels for computing the masked language modeling kd_loss. Indices should be in ``[-100, 0, ...,
        ...
        if tag == "mlm" and labels is not None:
            if (
        ...
        bias = None
        if self.n_ans and tag == "mlm" and labels is None:  # downstream mode
        ...
        if tag == "mlm":
            logits, loss = self.lm_predictions(modified[-1], embeddings=embeddings, bias=bias, labels=labels, tag=tag)
        else:
            logits, loss = self.lm_predictions(modified[-1][:, video.size(1):], mask_labels=mask_labels, labels=labels, tag=tag)
...
class DebertaV2LMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states, **kwargs):
        if kwargs["tag"] == "mlm":
            embedding_weight, bias = kwargs["embeddings"], kwargs["bias"]
            hidden_states = self.dense(hidden_states)
            hidden_states = self.transform_act_fn(hidden_states)
            hidden_states = self.LayerNorm(hidden_states)
            if bias is not None:
            ...
            mask_labels = kwargs["mask_labels"]
            ctx_states = hidden_states[:, 0, :]  # b x d
            ind_labels = mask_labels.bool()
            seq_states = hidden_states[ind_labels].view(hidden_states.size(0), -1, hidden_states.size(2))
            seq_states = self.dense((ctx_states.unsqueeze(1) + seq_states) / 2)  # b x max_atokens x d
            seq_states = self.LayerNorm(seq_states)
            logits = self.transform_act_fn(seq_states)
        return logits
...
class DebertaV2OnlyMLMHead(nn.Module):
    ...
    def forward(self, sequence_output,  **kwargs):
        logits = self.lm_head(sequence_output, **kwargs)
        loss = None
        if kwargs["tag"] == "mlm":
            if kwargs["labels"] is not None:
                ...
                loss = loss_fct(
                    ...
                    kwargs["labels"].view(-1),  # labels[labels > 0].view(-1)
                )
        else:
            mask_labels = kwargs["mask_labels"]
            logits = self.classifier(logits).squeeze(-1)  # b x max_atokens
            mask_labels = mask_labels[mask_labels.bool()] > 1
            if kwargs["labels"] is not None:
                mask_logits = logits.view(-1)[mask_labels]  # n_atokens
                labels = kwargs["labels"].repeat_interleave(logits.size(1))[mask_labels]
                mask_loss_fn = torch.nn.BCEWithLogitsLoss(reduction='sum')
                loss = mask_loss_fn(mask_logits, labels) / logits.size(0)
            else:
                mask_labels = mask_labels.view(logits.size())
                logits = (logits * mask_labels).sum(1) / mask_labels.sum(1).clamp(min=1)  # b
        return logits, loss