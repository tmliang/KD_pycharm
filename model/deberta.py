...
class DebertaV2OnlyRTDHead(nn.Module):
    ...
    def forward(self, hidden_states, mask_labels, labels=None):
        ...
        seq_states = self.LayerNorm(ctx_states.unsqueeze(1) + seq_states)    # b x max_atokens x d
        seq_states = self.dense(seq_states)
        seq_states = self.transform_act_fn(seq_states)

        logits = self.classifier(seq_states).squeeze(-1)    # b x max_atokens
        mask_labels = mask_labels[ind_labels] > 1
        mask_loss = torch.tensor(0).to(logits).float()
        if labels is not None:
            mask_logits = logits.view(-1)[mask_labels]   # n_atokens
            labels = labels.repeat_interleave(logits.size(1))[mask_labels]
            mask_loss_fn = torch.nn.BCEWithLogitsLoss(reduction='sum')
            mask_loss = mask_loss_fn(mask_logits, labels) / logits.size(0)
        else:
            mask_labels = mask_labels.view(logits.size())
            logits = (logits.sigmoid() * mask_labels).sum(1) / mask_labels.sum(1).clamp(min=1)    # b
        return logits, mask_loss


class DebertaV2ForRTD(DebertaV2PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids", r"predictions.decoder.bias"]
    ...
    def __init__(
        ...
        self.mask_predictions = DebertaV2OnlyRTDHead(config)
        self.features_dim = features_dim
        if freeze_rtd:
            for n, p in self.mask_predictions.named_parameters():
                if ft_ln and "LayerNorm" in n:
                    continue
                else:
                    p.requires_grad_(False)
    ...
    def forward(
        ...
        labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
            Labels for computing the masked language modeling kd_loss. Indices should be in ``[-100, 0, ...,
            config.vocab_size]`` (see ``input_ids`` docstring) Tokens with indices set to ``-100`` are ignored
            (masked), the kd_loss is only computed for the tokens with labels in ``[0, ..., config.vocab_size]``
        """
        ...
        text_hidden_states = modified[-1][:, video.size(1):]
        logits, mask_loss = self.mask_predictions(text_hidden_states, mask_labels, labels)

        if not return_dict:
            output = (logits,) + outputs[1:]
            return (
                ((mask_loss,) + output) if mask_loss is not None else output
            )
