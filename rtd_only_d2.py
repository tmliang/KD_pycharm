...
from model import G_model, D_model, joint_model, get_tokenizer
...
def evaluate(
    ...
    for i_batch, batch_dict in enumerate(
        ...
        for x in thresholds:
            if dataset_name == "ivqa":
                predicted = F.one_hot(
                    D_pred[:, :x], num_classes=answer_id_expanded.shape[-1]
                ).sum(1)
                agreeings[x] = (predicted * answer_id_expanded).max(1)[0]
            else:
                agreeings[x] = D_pred[:, :x] == answer_id_expanded[:, :x]

        for i, (qid, gt, pred) in enumerate(
                zip(qids, answer_id, D_pred)
        ):
            res[qid] = {
                "pred": pred.tolist(),
...
def main(args):
    ...
    for a, aid in dataloader_test.dataset.a2id.items():
        ...
        a2tok[aid] = tok
    a2tok = a2tok.to(model.device)
    model.set_answer_embeddings(a2tok, freeze_last=args.freeze_last)     # init answer embedding module
    if not args.eval:
        if dist.is_main_process():
