...
def train_one_epoch(
    ...
    for i_batch, batch_dict in enumerate(
        ...
        t_logits = torch.stack(t_logits).max(0).values
...
def main(args):
    ...
    if not args.eval:
        ...
        teacher1 = copy.deepcopy(model)
        teacher2 = copy.deepcopy(model)
        teacher1.eval()
        teacher2.eval()
        teacher1.load_state_dict(torch.load(args.teacher_load, map_location="cpu")["model"], strict=False)
        teacher2.load_state_dict(torch.load('ckpt/msvd/msvd_c.pth', map_location="cpu")["model"], strict=False)
        TQ = [teacher1, teacher2]
        if dist.is_main_process():
